...
@admin_blueprint.route('/migrate_drop_legacy_premake_table', methods=['GET', 'POST'])
def migrate_drop_legacy_premake_table():
    """
    Migration to drop the legacy 'premake' table that still exists in production.
    This table was replaced by the unified Product model with is_premake flag.
    """
    if request.method == 'GET':
        ...
        has_premake_table = 'premake' in tables

        return jsonify({
            'title': 'Drop Legacy Premake Table',
            'description': 'Remove the old "premake" table that was migrated to the unified Product model',
            'table_exists': has_premake_table,
            'warning': 'This will permanently remove the premake table. Ensure all data has been migrated to Product model first.'
        })

    # POST - Execute migration
    try:
        # Drop table using CASCADE to handle foreign key constraints
        db.session.execute(text('DROP TABLE IF EXISTS premake CASCADE'))
        db.session.commit()

        log_audit("MIGRATION", "Schema", details="Dropped legacy premake table")

        return jsonify({
            'success': True,
            'message': 'Successfully dropped legacy premake table'
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({
